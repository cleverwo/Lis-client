...
@connect(({ appRecord, vendor, vendorSearchInput, loading }) => ({
  vendor,
  vendorSearchInput,
  appRecord,
...
class VendorSearchInput extends Component {
  /**
   * 没有选中的情况下，输入框获取焦点，触发查询。
   * 主要是为了用户刚把鼠标放到输入框的时候，获取下数据。
   */
  onFocus = () => {
    const { vendorSearchInput: { current: { key, value } } } = this.props;
    if (!key) {
      this.onChange(value);
    }
  };
  ...
  onChange = (value, option) => {
    const {
      dispatch,
      vendorSearchInput: { timeout, current: { key } },
      initValue,
      clearInitValue,
    } = this.props;
    ...
    let tmpTimeout = setTimeout(() => {
      dispatch({
        type: 'vendor/fetch',
        payload: { vdCode: value },
        callback: () => {
          dispatch({
            type: 'vendorSearchInput/current',
            payload: { value: value },
          });
        },
      });
    ...
    dispatch({
      type: 'vendorSearchInput/timeout',
      payload: tmpTimeout,
    });
  };
