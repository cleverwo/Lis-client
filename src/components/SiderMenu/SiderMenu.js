...
export default class SiderMenu extends PureComponent {
  ...
  checkPermission(authority) {
    ...
    if (undefined == this.props.authorities) {
      return false;
    }
    if (authority instanceof Array) {
      return authority.some(value => authorities.indexOf(value) > -1);
    }
  ...
  getNavMenuItems = menusData => {
    ...
    return menusData
      .filter(item => item.name && !item.hideInMenu && this.checkPermission(item.authority))
      .map(item => {
  ...
  render() {
    const { logo, collapsed, onCollapse, intl } = this.props;
